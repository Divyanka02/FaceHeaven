$(document).ready(function(){
	...
	*/
	...
	$("body").delegate("#selectProduct","click",function(event){
		...
		event.preventDefault();
		$(".overlay").show();
		console.log("hello from select products : " + pid);
		$.ajax({
			url : "product_details.php",
			method : "POST",
			data : {get_selected_product:1,pid:pid},
			success : function(data){
				count_item();
				getCartItem();
	...
	*/
	...
	$("body").delegate("#wishproduct","click",function(event){
		var pid = $(this).attr("pid");
		event.preventDefault();
		$(".overlay").show();
		console.log("pid: ");
	...
	*/
	$("body").delegate(".qty","keyup",function(event){
		...
		$('.net_total').html("Sub Total : $ " +net_total);


		var final_total = 0;
		final_total = net_total + 100;
		$('.final_total').html("Total : "+ CURRENCY+ " " +final_total);
	...
	*/
	...
	$("body").delegate(".remove2","click",function(event){
		var remove = $(this).parent().parent().parent();
		var remove_id = remove.find(".remove").attr("remove_id_wish");
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{removeItemFromWish:1,rid:remove_id},
			success	:	function(data){
				$("#cart_msg").html(data);
				wishOutDetails();
			}
	...
	$("body").delegate(".update2","click",function(event){
		var update = $(this).parent().parent().parent();
		var update_id = update.find(".update").attr("update_id_wish");
		var qty = update.find(".qty").val();
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{updateWishItem:1,update_id:update_id,qty:qty},
			success	:	function(data){
				$("#cart_msg").html(data);
				wishOutDetails();
			}
	...
	*/
	...
	function wishOutDetails(){
		$('.overlay').show();
		   $.ajax({
			   url : "action.php",
			   method : "POST",
			   data : {Common:1,wishOutDetails:1},
			   success : function(data){
				   $('.overlay').hide();
				   $("#wish_checkout").html(data);
					   net_total();
					   final_total();
			   }
		   })
	...
	*/
	function net_total(){
		...
		$('.net_total').html("Sub Total : "+ CURRENCY+ " " +net_total);
	}
